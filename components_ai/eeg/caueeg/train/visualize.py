...
from .evaluate import calculate_class_wise_metrics
...
def draw_lr_search_record(learning_rate_record, use_wandb=False):
    ...
    ax.plot(train_accs[:, 0], train_accs[:, 1], "o", color="tab:red", alpha=0.6, label="Train")
    ax.plot(val_accs[:, 0], val_accs[:, 1], "o", color="tab:blue", alpha=0.6, label="Validation")
    ax.plot(midpoints[:, 0], midpoints[:, 1], "-", color="tab:purple", alpha=0.8, linewidth=1.0, label="Midpoint")
    ...
    ax.plot(starting_log_lr, np.max(midpoints), "o", color="cyan", alpha=0.8, linewidth=2.0, label="Start LR")
...
def draw_loss_plot(losses, lr_decay_step=None):
    ...
    if lr_decay_step is None:
        ...
        ax.vlines(lr_decay_step, 0, 1, transform=ax.get_xaxis_transform(), colors="m", alpha=0.5, linestyle="solid")
    else:
        x2 = np.arange(lr_decay_step, N, lr_decay_step)
        ax.vlines(x2, 0, 1, transform=ax.get_xaxis_transform(), colors="m", alpha=0.5, linestyle="solid")
...
def draw_heatmap(data, row_labels, col_labels, ax, draw_cbar=False, cbar_label="", imshow_kw=None, cbar_kw=None):
...
def annotate_heatmap(
    im, data=None, anno_format="{x:.2f}", text_colors=("black", "white"), threshold=None, text_kw=None
):
    ...
    kw = dict(horizontalalignment="center", verticalalignment="center", size="large")
    kw.update(text_kw)
...
def draw_roc_curve(score, target, class_label_to_name, use_wandb=False, save_path=None):
    ...
    for i, color in zip(range(n_classes), colors):
        ax.plot(
            ...
            color=color,
            lw=lw,
            label="{0} (area = {1:0.2f})" "".format(class_label_to_name[i], roc_auc[i]),
        )
